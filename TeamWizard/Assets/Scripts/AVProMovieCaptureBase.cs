...
public class AVProMovieCaptureBase : MonoBehaviour 
{
	...
	public virtual void PrepareCapture()
	{
		...
		Debug.Log("[AVProMovieCapture] Start Capture: (" + _targetWidth + "x" + _targetHeight + " @ " + ((int)_frameRate).ToString() + "fps " + _pixelFormat.ToString() + ") to file '" + _filePath + "' vcodec:'" + _codecName + "' audio device:'" + _audioDeviceName + "'");
