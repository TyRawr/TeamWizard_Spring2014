...
public class FacePlusEditorWindow : EditorWindow
{
	...
	void OnEnable ()
	{
		...
		options = new Options() {
			...
			livePreview = EditorPrefs.GetBool ("FacePlus.Options.LivePreview", true),
	...
	void OnDisable() {
		...
		EditorPrefs.SetBool ("FacePlus.Options.LivePreview", options.livePreview);
...
//		}
	...
	void LinkGUI() {
		...
		if(GUILayout.Button("Go to Blendshapes Example Scene"))
				...
				//EditorApplication.OpenScene ("Assets/Mixamo Face Plus/Examples/Example-BlendShape.unity");
			if(GUILayout.Button("Go to Joint-Based Example Scene"))
				OpenDemoScene("Assets/Mixamo Face Plus/Examples/Example-Joint.unity");
				//EditorApplication.OpenScene ("Assets/Mixamo Face Plus/Examples/Example-Joint.unity");
			EditorGUILayout.Space ();
			Divider ();
			EditorGUILayout.Space ();
		if (GUILayout.Button("Vist Face Plus Documentation Page"))
				Application.OpenURL("http://www.mixamo.com/faceplus?utm_source=faceplus&utm_medium=application&utm_campaign=version1.0.0.3");
		if (GUILayout.Button("Need Characters or Animations?"))
				Application.OpenURL("http://www.mixamo.com/?utm_source=faceplus&utm_medium=application&utm_campaign=version1.0.0.3");
	...
	void RealtimeCaptureGUI() {
		...
		options.livePreview = EditorGUILayout.Toggle (options.livePreview, GUILayout.Width (15));
		//	GUILayout.Label ("Record Audio:");
		//	options.recordAudio = EditorGUILayout.Toggle (options.recordAudio, GUILayout.Width (15));
		if (character != null) {
			if (options.mode == Mode.Realtime && options.livePreview && !character.Live) {
			...
			} else if (!options.livePreview && character.Live) {
		...
		if (GUILayout.Button("Check Minimum Requirements")){
				Application.OpenURL("http://www.mixamo.com/faceplus#requirements?utm_source=faceplus&utm_medium=application&utm_campaign=version1-requirements");
		}		
	}
	...
	{	
		...
		if (character == null && c != null) {
			if (options.livePreview && options.mode == Mode.Realtime){
